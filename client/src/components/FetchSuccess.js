...
export default function FetchProducts() {
  ...
  useEffect(() => {
    axios.get('http://localhost:9000/successdisplay').then((response) => {
  ...
  return (
    <>
      <section className="px-5 py-10 lg:py-20 xl:max-w-6xl xl:mx-auto grid grid-cols-1 gap-5 lg:gap-10 xl:gap-20" style = {{backgroundColor:"white",color:"black"}}>
        <h2 style={{fontSize:"40px",textAlign:"center",fontWeight:"600"}}>Our Success Stories</h2>
        {items===''?'': items.map(({story_name: name, description:desc, img1:small, img2:large }) => (
          <article
            //key={id}
